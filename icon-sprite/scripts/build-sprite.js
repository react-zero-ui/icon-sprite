...
function resolveLucideIconsDir() {
	const mergedDir = path.join(__dirname, "lucide-icons");
...
function loadLucideIconManifest() {
	const manifestFile = path.join(__dirname, "lucide-icons.json");
...
const lucideManifest = loadLucideIconManifest();
...
if (lucideManifest) {
	for (const icon of neededIcons) {
		...
		const svg = lucideManifest[icon.svgFile];
		if (svg) {
